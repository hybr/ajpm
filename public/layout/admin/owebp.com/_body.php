...
<div class="ui-widget-header ui-corner-all jpmHeaderPadding">
	...
	<?php } else { ?>
		<a title="Our Products and Services" href="/-a-item_catalog/present_all">Catalog</a>
		<a title="Items seleced for purchase" href="/-a-shopping_cart/present_all">Cart</a>
	<?php } ?>

	<?php if ($_SESSION ['allowed_as'] == 'PUBLIC' || $_SESSION ['allowed_as'] == 'NULL') {
		echo '<a title="'. $_SESSION['allowed_as'] . '" href="/-a-user/login">Login</a>';
	} else {
		echo '<a title="'. $_SESSION['allowed_as'] . '" href="/-a-user/logout">Logout</a>';
	} ?>

	<a title="Join and manage you business" href="/-a-user/join">Join</a>
	...
	if ($_SESSION ['allowed_as'] != 'PUBLIC' && $_SESSION ['allowed_as'] != 'NULL') {
		...
		echo '<a href="/-a-user/update?id='.(string)$_SESSION ['user']['_id'] 
			.'">Welcome</a>';
		if ($personName != '') {
			echo ' <a href="/-a-person/update?id='.$_SESSION ['login_person_id']
				.'">'.$personClass->getFullName('Official').'</a>';
		}
	}
	?>		
	<a href="/-a-contact/present_all">Contact Us</a>
</div></div><br />
